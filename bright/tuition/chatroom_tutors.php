...
<?php
$student_id = $_GET['id'];
$result = mysqli_query($con,"select name from students where student_id='$student_id'");
while($row = mysqli_fetch_assoc($result))
{
...
<nav>
    <div class="nav-wrapper grey lighten-5">
      ...
      <li><a href="#notifications.php" style=" text-decoration:none"><i class="material-icons" style="color:black;">notifications</i></a></li></a>
        ...
        <li><a href="users/tutors/logout.php" class="black-text" style=" text-decoration:none" >Logout</a></li>
      </ul>
    </div>
  </nav>
<!-- Chat-Area -->
<h4>Chatting with Student</h4><h6><?php echo strtoupper($student_name);?></h6>
  ...
  <div class="row">
    ...
    <!-- </form> -->
        ...
        <div class="card-content">
          <p>
          <?php
           $result = mysqli_query($con,"select message from chat where to_id=$tutor_id AND from_id=$student_id AND from_person='$student_name'");
            while($row = mysqli_fetch_assoc($result))
            {
              $msg = $row['message'];
              echo " <p style='color:black'> from $student_name--->$msg </p>"; 
            }
...
{
  $msg = $_POST['message'];
  $query = "INSERT INTO chat (from_person,from_id,to_person,to_id,message) VALUES ('$name','$tutor_id','$student_name','$student_id','$msg')";
